...
# ⭐ NOVOS IMPORTS NECESSÁRIOS PARA O AJAX:
...
# ===============================
# ⭐ FUNÇÃO AJAX PARA FAVORITOS (TOGGLE)
# ===============================
...
def toggle_favorito(request):
    """
    Endpoint AJAX para adicionar ou remover uma oportunidade dos favoritos do usuário.
    """
    oportunidade_id = request.POST.get('oportunidade_id') 

    if not oportunidade_id:
        return JsonResponse({'status': 'error', 'message': 'ID da oportunidade ausente'}, status=400)

    try:
        # Garante que a oportunidade realmente existe
        oportunidade = get_object_or_404(Oportunidade, id=oportunidade_id)
        
        # Tenta encontrar o registro de Favorito para este usuário e oportunidade
        try:
            favorito = Favorito.objects.get(usuario=request.user, oportunidade=oportunidade)
            
            # Se encontrado, remove (desmarca)
            favorito.delete()
            is_saved = False
            message = "Oportunidade removida dos favoritos."
        except Favorito.DoesNotExist:
            # Se não encontrado, cria (salva)
            Favorito.objects.create(usuario=request.user, oportunidade=oportunidade)
            is_saved = True
            message = "Oportunidade salva nos favoritos."

        return JsonResponse({'status': 'success', 'is_saved': is_saved, 'message': message})
...
        # ⭐ AJUSTE AQUI: Otimiza para usar 'oportunidade_id'
...
    context_object_name = 'oportunidades_salvas' # Variável usada no template
    login_url = 'login'
    
    def get_queryset(self):
        # ⭐ AJUSTE AQUI: Retorna os objetos Favorito para facilitar o acesso no template
        # e é mais eficiente, pois evita carregar e depois descartar os objetos
        favoritos = Favorito.objects.filter(
            usuario=self.request.user
        ).select_related('oportunidade').order_by('-data_adicionado')
        
        return favoritos # Retorna a lista de objetos Favorito
    
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # O get_queryset retorna Favoritos, então contamos por lá
        context['total_favoritos'] = self.get_queryset().count() 
        return context
    
    # ⭐ REMOVIDO o método post, pois o AJAX toggle_favorito() cuida da remoção.